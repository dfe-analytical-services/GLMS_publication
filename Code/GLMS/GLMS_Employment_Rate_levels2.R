...
#The all datasets are quarterly datasets modified in 2.
  ...
  Headline_stats_function<-function(all,agebracket){
    ...
    Headline_stats<-data.frame( in_employment$Graduate_Type,                              #names of population for each grad type
                                ...
                                inactive$sample,                                            #not_employed sample
                                ...
                                inactive$count,                                       # not emplyed total
  ...
  #full_pop$total total population for each grad type   
    ...
    colnames(Headline_stats)<-c("Population","Employment_rate","High_skill_emp_rate","Unemployment_rate","Inactivity_rate",
                                "Employed sample","HS employed sample", "unemployed sample", "inactive sample",
                                "employed total","HS employed total","unemployed total","inactive total",
                                "full_population_total","full_population_sample", "Age_range")
    ...
    Headline_stats<-Headline_stats[c("Age_range","Population","Employment_rate","High_skill_emp_rate","Unemployment_rate","Inactivity_rate", 
                                     "Employed sample","HS employed sample", "unemployed sample", "inactive sample",
                                     "employed total","HS employed total", "unemployed total","inactive total", 
                                     "full_population_total","full_population_sample")]
  }
...
#the most recent people weight is picked and samples/counts for defined population is found
  ...
  headline_stats$employ_accuracy_flag <- ifelse(headline_stats$`Employed sample` <= 30, 1,0)
  headline_stats$hse_accuracy_flag <- ifelse(headline_stats$`HS employed sample` <= 30, 1,0   )
  headline_stats$unemploy_accuracy_flag <- ifelse(headline_stats$`unemployed sample` <= 30, 1,0   )
  headline_stats$Inactive_accuracy_flag <- ifelse(headline_stats$`inactive sample` <= 30, 1,0   )
  
  headline_stats$employ_suppression_flag <- ifelse(headline_stats$`Employed sample` <= 4, 1,0)
  headline_stats$hse_suppression_flag <- ifelse(headline_stats$`HS employed sample` <= 4, 1,0   )
  headline_stats$unemploy_suppression_flag <- ifelse(headline_stats$`unemployed sample` <= 4, 1,0   )
  headline_stats$Inactive_suppression_flag <- ifelse(headline_stats$`inactive sample` <= 4, 1,0   )
...
#quarterly datasets combined to yearly datasets, find mean rates and total sample sizes/ counts for a year
  ...
  year_data%>%
    ...
    summarise(Employment_rate=mean.default(Employment_rate), 
              ...
              full_population_sample=sum(`full_population_sample`),
              employ_accuracy_flag = sum(employ_accuracy_flag),
              hse_accuracy_flag = sum(hse_accuracy_flag),
              unemploy_accuracy_flag = sum(unemploy_accuracy_flag),
              Inactive_accuracy_flag = sum(Inactive_accuracy_flag),
              employ_suppression_flag = sum(employ_suppression_flag),
              hse_suppression_flag = sum(hse_suppression_flag),
              unemploy_suppression_flag = sum(unemploy_suppression_flag),
              Inactive_suppression_flag = sum(Inactive_suppression_flag)
             ) -> year_average
...
#breakdowns
 ...
 Graduate_breakdown$employ_accuracy_flag <- ifelse(Graduate_breakdown$Employed_sample <= 30, 1,0   )
 Graduate_breakdown$hse_accuracy_flag <- ifelse(Graduate_breakdown$High_Skill_employed <= 30, 1,0   )
 Graduate_breakdown$unemploy_accuracy_flag <- ifelse(Graduate_breakdown$Unemployed_sample <= 30, 1,0   )
 Graduate_breakdown$Inactive_accuracy_flag <- ifelse(Graduate_breakdown$Inactive_sample <= 30,1,0   )
  
 Graduate_breakdown$employ_suppression_flag <- ifelse(Graduate_breakdown$Employed_sample <= 4, 1,0   )
 Graduate_breakdown$hse_suppression_flag <- ifelse(Graduate_breakdown$High_Skill_employed <= 4, 1,0   )
 Graduate_breakdown$unemploy_suppression_flag <- ifelse(Graduate_breakdown$Unemployed_sample <= 4, 1,0   )
 Graduate_breakdown$Inactive_suppression_flag <- ifelse(Graduate_breakdown$Inactive_sample <= 4,1,0   )
...
Graduate_breakdown_year<-function(year){
  ...
  samples<-ddply(year_data, .(Group,Population, Breakdown),colwise(sum, .(Employed_sample, 
                                                                          ...
                                                                          employment_total, high_skill_employment_total,
                                                                          unemployment_total, inactive_total, full_population_total,full_population_sample,
                                                                          employ_accuracy_flag, hse_accuracy_flag, unemploy_accuracy_flag, Inactive_accuracy_flag,
                                                                          employ_suppression_flag, hse_suppression_flag, unemploy_suppression_flag, Inactive_suppression_flag),na.rm=TRUE))
  ...
  output<-output[c("Group",
                   ...
                   "employ_accuracy_flag",
                   "hse_accuracy_flag",
                   "unemploy_accuracy_flag",
                   "Inactive_accuracy_flag",
                   "employ_suppression_flag",
                   "hse_suppression_flag",
                   "unemploy_suppression_flag",
                   "Inactive_suppression_flag"
                   )]
