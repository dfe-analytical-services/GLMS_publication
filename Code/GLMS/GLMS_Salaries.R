...
Average_salary_by<-function(dataset,demographic){
  ...
  dataset%>%
    ...
    dplyr::summarise(p25=weighted.quantile(GRSSWK, !!sym(weight), probs=0.25, type =4, collapse = TRUE),
                     ...
                     p25_real=weighted.quantile(GRSSWK_real, !!sym(weight), probs=0.25, type =4, collapse = TRUE),
                     p90_real=weighted.quantile(GRSSWK_real, !!sym(weight), probs=0.90, type =4, collapse = TRUE),
                     median_real=weighted.median(GRSSWK_real,!!sym(weight), type = 4, collapse = TRUE),
                     mean_real=weighted.mean(GRSSWK_real,!!sym(weight)),
                     sample=n(),
                     population=sum(!!sym(weight)))%>% 
    as.data.frame()%>%  
    mutate(variable=paste0(demographic, collapse = "_"))
  
  #take a dataset; based on the demographic characteristic separate the dataet into groups; find the quartiles and median of GRSSWK (gross weekly pay) and weight on PIWT16
...
#Add in an 'agegroup' for the average weekly salaries by graduate type and rename the column headers: this way we can merge the datasets as they'll have the same headers
  ...
  #Add flags for accuracy and suppression
  all_sal$accuracy_flag <- ifelse(all_sal$sample <= 30, 1,0)
  all_sal$suppression_flag <- ifelse(all_sal$sample <= 4,1,0)
...
#Average salary by industry category
  ...
  #append datasets to each other to create one dataset of graduate salaries for different demographic characteristics
      ...
      grad_sal$accuracy_flag <- ifelse(grad_sal$sample <= 30, 1,0)
      grad_sal$suppression_flag <- ifelse(grad_sal$sample <= 4,1,0)
  ...
  #append datasets to each other to create one dataset of graduate salaries for different demographic characteristics
      ...
      young_grad_sal$accuracy_flag <- ifelse(young_grad_sal$sample <= 30, 1,0)
      young_grad_sal$suppression_flag <- ifelse(young_grad_sal$sample <= 4,1,0)
